...
def loaddata(filename):
    ...
    :return:
    """
    mat = scio.loadmat(os.path.join('datasets', filename))
    X_orig = mat['X']
    y_orig = mat['y'].ravel()
...
def print_save_result(data, base_detector, n_baselines, roc_mat,
                      ap_mat, method_list, timestamp, verbose,res_path='results'):
    """
    :param data:
    :param base_detector:
    :param n_baselines:
    ...
    f = open(
        res_path+'\\' + data + '_' + base_detector + '_' + timestamp + '.csv',
        'a')
